...
vpc_id = aws_vpc.demo-vpc-uc10.id
  tags = {
    Name = "demo-vpc-uc10-igw"
  }
}
...
resource "aws_security_group" "focal_board_alb" {
  name   = "focal_board_alb"
  vpc_id = aws_vpc.demo-vpc-uc10.id
...
resource "aws_security_group" "instance_sg" {
  ...
  ingress {
    ...
    to_port     = 8000
    protocol    = "tcp"
    security_groups = [aws_security_group.focal_board_alb.id]
  }
